...
export function wikidataBasedSiteUrl (wdBasedSite, id) {
  return wikidataBasedSites[wdBasedSite].urlFormat.replace('$1', id)
}

export function findFirstWikidataBasedSite (sites) {
  for (const site of sites) {
    if (site === 'wikidata' || wikidataBasedSites[site]) return site
  }
}

export function replaceWikidataBaseSiteByWikidata (site) {
  if (wikidataBasedSites[site] != null) return 'wikidata'
  return site
}

export function addWikidataBasedSitesLinksData (data, id, shortlist) {
