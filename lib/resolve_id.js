...
export async function resolveId (id, context) {
  context.origin = { id }
  ...
  info('origin properties', properties)
  info('origin value', value)
  ...
  const url = wdk.getReverseClaims({
    properties,
    values: value,
  ...
  const { parsedBody } = await get(url)
  const results = wdk.simplify.sparqlResults(parsedBody)
  const ids = results.map(result => result.subject)
  if (ids.length > 0) {
    context.origin.qid = ids[0]
    return ids[0]
  } else {
    throw newError('no id found', 400, { properties, value })
  }
}
